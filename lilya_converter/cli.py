"""CLI application for FastAPI-to-Lilya conversion.
...
app = Sayer(
    ...
    help="Convert FastAPI projects into Lilya projects using source-grounded rules.",
...
@app.command(
    help=(
        "Analyze a FastAPI project and print conversion-relevant findings "
        "(apps, routers, routes, dependencies, middleware/events).\n\n"
        "Examples:\n"
        "  lilya-converter analyze ./fastapi_app\n"
        "  lilya-converter analyze ./fastapi_app --json\n"
...
def analyze(
    ...
    """Analyze a FastAPI project and emit a scan report.

    Args:
        source: Path to the FastAPI project root.
    ...
    for module in report.modules:
        info(
            ...
            f"include_router={len(module.include_routers)}"
        )
...
@app.command(
    help=(
        "Convert a FastAPI project into a Lilya project using deterministic, "
        "repo-grounded AST transformation rules.\n\n"
        "Examples:\n"
        "  lilya-converter convert ./fastapi_app ./lilya_app\n"
        "  lilya-converter convert ./fastapi_app ./lilya_app --dry-run --diff\n"
...
def convert(
    ...
    Args:
        source: Source FastAPI project root.
...
@app.command(
    help=(
        "Create a minimal Lilya scaffold informed by scanned FastAPI structure.\n\n"
        "Examples:\n"
        "  lilya-converter scaffold ./fastapi_app ./lilya_scaffold\n"
        "  lilya-converter scaffold ./fastapi_app ./lilya_scaffold --dry-run"
    ),
)
def scaffold(
    source: Annotated[str, Argument(help="FastAPI project root to inspect.")],
    ...
    Args:
        source: FastAPI source project root used for structural detection.
        target: Destination directory for scaffolded Lilya files.
        dry_run: If `True`, report scaffold actions without writing files.
    """
    report = scaffold_project(source_root=source, target_root=target, dry_run=dry_run)
    info(f"Source: {report.source_root}")
...
@map_app.command(
    "rules",
    help=("List all available conversion rule identifiers and summaries.\n\nExample:\n  lilya-converter map rules"),
)
def map_rules() -> None:
    """Print the known conversion rule registry."""
    for rule_id, summary in mapping_rules():
...
@app.command(
    help=(
        "Run post-conversion checks (syntax, remaining FastAPI patterns, "
        "unresolved local imports) on a target Lilya project.\n\n"
        "Examples:\n"
        "  lilya-converter verify ./lilya_app\n"
        "  lilya-converter verify ./lilya_app --report ./reports/verify.json"
    ),
)
def verify(
